...
namespace DiscordRPC
{
    ...
    public sealed class DiscordRpcClient : IDisposable
    {
        ...
        public DiscordRpcClient(string clientId) : this(clientId, -1) { }
		...
		private void ProcessMessage(IMessage message)
        {
            ...
            switch (message.Type)
            {
                ...
                case MessageType.Authorized:
                    {
                        var msg = message as AuthorizedMessage;
                ...
                case MessageType.Authenticated:
                    {
                        var msg = message as AuthenticatedMessage;
            ...
            switch (message.Type)
            {
                ...
                case MessageType.GotVoiceSettings:
                    if (OnVoiceSettingsUpdate != null) OnVoiceSettingsUpdate.Invoke(this, message as VoiceSettingsMessage);
                    break;
        ...
        public void DoStandardPrecheck(bool treatNoInitializedAsException)
        {
            ...
            if (!IsInitialized)
            {
                if (treatNoInitializedAsException)
                {
                    throw new UninitializedException();
                }
                else
                {
                    Logger.Warning("The client is not yet initialized, storing as a state instead.");
                }
            }
        ...
        public void GetVoiceSettings()
        {
            ...
            //Send the event
            connection.EnqueueCommand(new GetVoiceSettingsCommand());
        }
        ...
        public RichPresence UpdateStartTime(DateTime time)
        {
            ...
            lock (_sync)
            {
                ...
                if (CurrentPresence.Timestamps == null)
                {
        ...
        private void SubscribeToTypes(EventType type, bool isUnsubscribe)
        {
            ...
            if ((type & EventType.VoiceSettingsUpdate) == EventType.VoiceSettingsUpdate)
            {
                connection.EnqueueCommand(new SubscribeCommand() { Event = RPC.Payload.ServerEvent.VoiceSettingsUpdate, IsUnsubscribe = isUnsubscribe });
            }
        }
